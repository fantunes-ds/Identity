...
void Engine::Rendering::Buffers::InputLayout::Generate(const Microsoft::WRL::ComPtr<ID3D11Device>& p_device, const Microsoft::WRL::ComPtr<ID3DBlob>& p_blob)
{
    HRESULT hr;
    ...
    GFX_THROW_INFO(p_device->CreateInputLayout(m_inputDesc.data(),
                                               std::size(m_inputDesc),
                                               p_blob->GetBufferPointer(),
                                               (UINT)p_blob->GetBufferSize(),
                                               &m_inputLayout));
}

void Engine::Rendering::Buffers::InputLayout::Bind(const Microsoft::WRL::ComPtr<ID3D11DeviceContext>& p_context) const
{
