...
std::shared_ptr<ObjectElements::Model> ObjectLoader::LoadModel(const std::string& p_file)
{
...
void ObjectLoader::ParseForNodes(const aiNode*          p_assimpNode, std::shared_ptr<Scene::SceneNode> p_sceneNode,
                                 ObjectElements::Model& p_model)
{
    if (!p_sceneNode->IsRoot())
    {
        // auto newTransform = std::make_shared<Components::Transform>();
        int32_t       id    = Systems::TransformSystem::AddTransform();
        GPM::Matrix4F local = Tools::ASSIMP::ASSIMPConversion::Mat4x4ToGPM(p_assimpNode->mTransformation);
        Systems::TransformSystem::GetTransform(id)->SetLocalTransformMatrix(local);
...
std::shared_ptr<ObjectElements::Mesh> ObjectLoader::LoadMesh(aiMesh* p_assimpMesh)
{
    ...
    for (unsigned int vertIdx = 0u; vertIdx < p_assimpMesh->mNumVertices; vertIdx++)
    {
        ...
        if (p_assimpMesh->HasTextureCoords(0))
        {
            ...
            vertex.m_textCoords = Vector2D(UV.x, UV.y);
        }

        vertex.m_position = Vector3D(vert.x, vert.y, vert.z);
        vertex.m_normal   = Vector3D(norm.x, norm.y, norm.z);
